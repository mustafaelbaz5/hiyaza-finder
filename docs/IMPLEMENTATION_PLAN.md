...
| 2.1 | Extract `ParcelQueryService`, `ParcelEditOverlay`, `BulkEditService`, `ClipboardFormatter` out of `HoldingsRepository` (behavior-preserving) | Same search results, same copy-all output, same bulk-edit behavior — new unit tests on all four, including the وراثة/مفوض prefix matrix |
