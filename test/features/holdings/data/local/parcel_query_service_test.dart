...
void main() {
  ...
  final List<Parcel> parcels = <Parcel>[
    const Parcel(
        id: '1', holdingId: '101', holderName: 'محمد علي', basinName: 'البشيط'),
    const Parcel(
        id: '2', holdingId: '101', holderName: 'محمد علي', basinName: 'البشيط'),
    const Parcel(
        id: '3',
        holdingId: '102',
  ...
  group('pending (not-yet-numbered) new people don\'t collide', () {
    ...
    test('basinHoldingCounts counts them as two separate holdings', () {
      final Map<String, int> counts =
          service.basinHoldingCounts(pendingParcels);
  ...
  group('basinSummaries', () {
    test('counts a holding as completed only when every one of its parcels is',
      ...
      final List<Parcel> mixed = <Parcel>[
        ...
        const Parcel(
            id: '2', holdingId: '102', basinName: 'الباشا'), // not completed
        const Parcel(id: '3', holdingId: '103', basinName: 'البحيره'),
      ];

      final List<BasinProgress> summaries = service.basinSummaries(mixed);
      final BasinProgress basha = summaries
          .firstWhere((final BasinProgress b) => b.basinName == 'الباشا');
      expect(basha.totalCount, 2);
      expect(basha.completedCount, 1);
      expect(basha.isFullyCompleted, isFalse);

      final BasinProgress bahira = summaries
          .firstWhere((final BasinProgress b) => b.basinName == 'البحيره');
    ...
    test('a multi-parcel holding only counts as completed once every parcel is',
      ...
      final List<Parcel> multiParcelHolding = <Parcel>[
        ...
        const Parcel(
            id: '2',
            holdingId: '101',
            basinName: 'الباشا'), // same holding, not done
      ];

      final BasinProgress basha =
          service.basinSummaries(multiParcelHolding).single;
    ...
    test('is sorted by basin name', () {
      ...
      expect(summaries.map((final BasinProgress b) => b.basinName),
          <String>['أ', 'ب']);
    });
  });
  ...
  group('adjacentHoldingGroupKey', () {
    ...
    test('next/previous follow رقم الحيازة ascending order, not list order',
        () {
      expect(
        service.adjacentHoldingGroupKey(basinParcels, 'الباشا', '1',
            next: true),
        '2',
      );
      expect(
        service.adjacentHoldingGroupKey(basinParcels, 'الباشا', '2',
            next: true),
        '3',
      );
      expect(
    ...
    test('returns null at either end', () {
      ...
      expect(
        service.adjacentHoldingGroupKey(basinParcels, 'الباشا', '3',
            next: true),
        isNull,
      );
    });
    ...
    test('returns null for a groupKey not in the given basin', () {
      expect(
        service.adjacentHoldingGroupKey(basinParcels, 'حوض آخر', '1',
            next: true),
        isNull,
      );
    });
  ...
  group('findByBorderText', () {
    ...
    test(
        'matches a حائز name exactly and returns null for non-matches/non-person text',
        () {
      expect(service.findByBorderText(index, 'محمد على')!.holdingId, '101');
      expect(service.findByBorderText(index, 'محمد'),
          isNull); // no substring match
    ...
    test('ambiguous name resolves to the holding with the most parcels', () {
      ...
      expect(service.findByBorderText(ambiguousIndex, 'علي حسن')!.holdingId,
          '202');
    });
  });
}
