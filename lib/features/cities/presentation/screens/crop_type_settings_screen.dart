...
class CropTypeSettingsScreen extends StatefulWidget {
  ...
  @override
  State<CropTypeSettingsScreen> createState() =>
      _CropTypeSettingsScreenState();
}

class _CropTypeSettingsScreenState extends State<CropTypeSettingsScreen> {
  final CropTypeRepository _repository = getIt<CropTypeRepository>();
  ...
  Future<void> _remove(final String cropType) async {
    ...
    try {
      ...
      if (!mounted) return;
      setState(() {
        _cropTypes = _cropTypes.where((final String c) => c != cropType).toList();
