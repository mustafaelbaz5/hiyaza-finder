...
class _AddRecordScreenState extends State<AddRecordScreen> {
  ...
  Parcel get _parcelToSave {
    final ParcelCatalogRepository repo = widget.resolvedRepository;
  ...
  Future<void> _save() async {
    ...
    setState(() {});
    try {
      final Parcel? saved = await widget.resolvedRepository.addLocalParcel(
        _parcelToSave,
  ...
  Parcel _applyHoldingId(final String value) {
    ...
    final ParcelCatalogRepository repository = widget.resolvedRepository;
  ...
  Future<void> _editBasin(final BuildContext context) async {
    final List<String> basins = widget.resolvedRepository.availableBasins;
    if (basins.isEmpty) {
      await _editText(
        context,
                ...
                if (_awaitingExistingPersonSearch)
                  Expanded(
                    child: ExistingPersonSearch(
                      repository: widget.resolvedRepository,
