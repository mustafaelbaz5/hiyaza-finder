...
class AppControlRemoteDataSource {
  ...
  Future<AppControl> read() async {
    final Uri uri = Uri.parse('${AppConfig.supabaseUrl}/rest/v1/app_control').replace(
    ...
    if (response.statusCode < 200 || response.statusCode >= 300) {
      throw http.ClientException('App control request failed: ${response.statusCode}', uri);
    }
    final List<dynamic> rows = jsonDecode(response.body) as List<dynamic>;
    return rows.isEmpty ? AppControl.open : AppControl.fromJson(rows.first as Map<String, dynamic>);
  }
}
